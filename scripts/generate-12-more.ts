#!/usr/bin/env npx tsx
/**
 * 추가 12개 도시 아티클 생성 (38 → 50개)
 * env $(cat .env.local | grep -v '^#' | xargs) npx tsx scripts/generate-12-more.ts
 */
...
다음 JSON 형식으로 응답해주세요 (마크다운 코드블록 없이 순수 JSON만):
{
  ...
  "content": "마크다운 형식의 본문 (2000자 이상, 한국어). ## 소제목 사용, 추천 장소/음식/팁 포함. ### 세부 항목도 사용. 1인칭 표현 없이 객관적 정보 전달.",
