...
export default function EnCommentSection({ postId, currentUserId }: { postId: string; currentUserId: string | null }) {
  ...
  return (
    <div className="mt-8">
      <h3 className="font-bold text-lg mb-4">{comments.length} {comments.length === 1 ? 'Comment' : 'Comments'}</h3>
      ...
      {!replyTo && (
        user ? (
          ...
          <p className="text-sm text-gray-500 text-center py-4 bg-gray-50 rounded-[var(--radius)]">
            <a href="/auth/login" className="text-[var(--primary)] underline">Log in</a> to leave a comment.
          </p>
        )
      )}
    </div>
    ...
    <div className="bg-gray-50 rounded-[var(--radius)] p-3">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <span className="font-medium text-gray-700">{comment.user_profiles?.username ?? 'Anonymous'}</span>
          <span>{timeAgo}</span>
        </div>
        {isOwner && !editing && (
