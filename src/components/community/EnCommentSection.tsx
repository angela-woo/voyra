...
export default function EnCommentSection({ postId, currentUserId }: { postId: string; currentUserId: string | null }) {
  ...
  const handleEdit = async (commentId: string, newContent: string) => {
    ...
    if (!res.ok) { toast.error('Failed to update comment.'); return false }
    fetchComments()
    return true
  }
  ...
  return (
    <div className="mt-8">
      <h3 className="text-lg mb-4" style={{ fontFamily: 'var(--font-heading)', fontWeight: 700 }}>{comments.length} {comments.length === 1 ? 'Comment' : 'Comments'}</h3>
      ...
      {!replyTo && (
        user ? (
          ...
          <p className="text-sm text-[color:var(--ink-faint)] text-center py-4 border border-[var(--border)]">
            <a href="/auth/login" className="text-[color:var(--primary)] underline">Log in</a> to leave a comment.
          </p>
        )
      )}
    </div>
...
function CommentItem({
  ...
  const timeAgo = formatDistanceToNow(new Date(comment.created_at), { addSuffix: true, locale: enUS })
    ...
    <div className="border border-[var(--border)] p-3">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2 text-xs text-[color:var(--ink-faint)]">
          <span className="text-[color:var(--ink-soft)]">{comment.user_profiles?.username ?? 'Anonymous'}</span>
          <span>{timeAgo}</span>
        </div>
        {isOwner && !editing && (
